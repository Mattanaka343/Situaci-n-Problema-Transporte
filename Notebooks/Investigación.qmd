...
Como primera DAG está basada en nuestras hipótesis. Propusimos el modelo preliminar en el cual definimos lo siguiente: La edad y el sexo son los nodos raíz e independientes entre ellos. El estrato socioeconómico esta dado por la edad y el sexo; este mismo se relaciona con la cantidad de vehiculos privados que la persona posee y define la población dentro de su localidad. La cantidad de vehiculos y la localidad informa el vehículo que se utiliza en un viaje. Finalmente, el día del viaje esta dado por el vehículo que se usa en dicho viaje
...
### Comparando las DAGS
Ahora antes de decidir que estructura utilizaremos para contestar nuestras preguntas compararemos las dags obteniendo sus puntajes en las pruebas de BIC (Bayesian Information Criterion) y AIC (Akaike Information Criterion).
...
Para entender propiamente como es que los factores socioeconomicos afectan los patrones de transporte de la población formulamos las siguientes preguntas
1. ¿Cuál es la probabilidad de que una mujer joven (adultez joven y adolscencia) de estrato social alto use la bicicleta?
```{r}
bn_mle = bn.fit(dag, data =data, method = 'mle')
cpquery(bn_mle,(V=='7'),((S=='2')&((E=='2')|(E=='1'))&(C=='4')))
```
...
```
  - la probabilidad de que en una comunidad pequeña sea de estrato alto es relativamente alta en aproximadamente 1 de cada 3 personas cumplen con estas caracteristicas 

2. ¿Cuál es la probabilidad de que alguien de estrato sociodemográfico bajo (bajo y medio bajo) tenga ya sea una moto, un auto o una bicicleta?
```{r}
cpquery(bn_mle, event = (A==1 | M==1 | B==1), evidence = (E==1 | E==2), n = 10^6)
```

  - La probailidad de encontrar un evento de este tipo es bastante alta, de alrededor de 44%. Mostrando que bastantes personas de estrato social bajo poseén vehículos privados. 
